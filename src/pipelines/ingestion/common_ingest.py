...
from pipelines.normalization.offline_graph_artifacts import (
    ...
    enrich_feedback_cache_first,
    extract_entities_cache_first,
    resolve_graphrag_cache_root,
)
...
def process_restaurant_sources(
    ...
    *,
    ...
    cache_root = resolve_graphrag_cache_root(
        settings.root.parent,
        settings.root.parent / "cache" / ".cache" / "graphrag",
    )
    module_config = replace(load_config(settings.root.parent), cache_dir=cache_root)
    if mode == "offline":
        feedback = enrich_feedback_cache_first(prepared.feedback, cache_root, module_config)
    ...
    if mode == "offline":
        extracted_entities, extracted_relations = extract_entities_cache_first(text_units, cache_root, module_config)
