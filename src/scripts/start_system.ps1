...
$productionRoot = Join-Path $repoRoot "production"
...
$pythonExe = if ((Test-Path $venvPython) -and (Test-PythonExecutable $venvPython)) {
    $venvPython
} elseif (Test-PythonExecutable "python") {
    "python"
} else {
    throw "No usable Python executable found. Recreate .venv or install Python and add it to PATH."
}
...
try {
    ...
    if ($RebuildOffline) {
        Write-Host "Rebuilding offline data artifacts..."
            Invoke-Checked "offline ingest" { & $pythonExe (Join-Path $srcRoot "scripts/run_offline_ingest.py") --config (Join-Path $productionRoot "configs/offline.yaml") }
            Invoke-Checked "KG build" { & $pythonExe (Join-Path $srcRoot "scripts/build_kg.py") --mode offline }
            Invoke-Checked "vector index build" { & $pythonExe (Join-Path $srcRoot "scripts/build_indexes.py") --mode offline }
            Invoke-Checked "R-GCN snapshot export" { & $pythonExe (Join-Path $srcRoot "scripts/export_rgcn_snapshot.py") --mode offline }
    }
